...
const laptopFor = (track) => /pipegen/i.test(track || '')
  ? 'Bring your own laptop with Linux and an NVIDIA GPU, plus your chargers.'
  : /firmgen/i.test(track || '')
  ? 'Bring your own laptop and chargers. Any config works for FirmGen.'
...
function approvedEmail(reg) {
  const body = para(`Hi ${firstName(reg)},`)
    ...
    + para(`You are in on the <b style="color:#0f1b2d">${esc(reg.track || 'build')}</b> track. We will provide ${kitFor(reg.track)} and mentors on the floor. ${laptopFor(reg.track)}`)
...
function approvedText(reg) {
  return `Hi ${(reg.fullName || 'there').split(' ')[0]},\n\n`
    ...
    + `You are in on the ${reg.track || 'build'} track. We will provide ${kitFor(reg.track)} and mentors on the floor. ${laptopFor(reg.track)}\n\n`
    + `When: ${EVENT.date}, ${EVENT.time}\nWhere: ${EVENT.venue}\n\n`
