  ...
  <tr><td align="center">
    <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden">
      <tr><td style="padding:24px 40px 18px;border-bottom:1px solid #eef0f2">
        ...
        <div style="font-size:12px;color:#8b95a3;margin-top:3px">Embedded Hardware + AI Buildathon &middot; CraftifAI &times; DevAarambh</div>
      </td></tr>
...
const kitFor = (track) => (track === 'PipeGen' ? 'an NVIDIA Jetson' : track === 'FirmGen' ? 'ESP32 and STM32 boards' : 'your hardware');
...
const sendPending = (reg) => sendEmail(reg.email, 'Your CircuitX registration has been received', pendingEmail(reg), pendingText(reg));
const sendApproved = (reg) => sendEmail(reg.email, 'Your CircuitX seat is confirmed', approvedEmail(reg), approvedText(reg));
