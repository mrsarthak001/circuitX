...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === 'GET') {
      ...
      return res.status(200).json({ ok: true, firstName: (rec.fullName || '').split(' ')[0], track: rec.track || '', status: rec.status, rsvp: rec.rsvp });
    }
    ...
    if (req.method === 'POST') {
      ...
      if (!rows.length) return res.status(404).json({ ok: false, error: 'invalid or expired link' });
      const rec = mapRow(rows[0]);
      return res.status(200).json({ ok: true, rsvp: rec.rsvp, firstName: (rec.fullName || '').split(' ')[0] });
    }
